...
**Catatan penting untuk 3 method `show()` (Produk/Artikel/Portfolio)**: Route publik memakai *implicit route-model binding* (`{product:slug}`, `{article:slug}`, `{portfolioProject:slug}`) — Laravel MENGAMBIL model utama dari database SEBELUM method controller dijalankan, di luar kendali `Cache::remember` di dalam method. Ini pola idiomatik Laravel ("Do Things the Laravel Way") dan SENGAJA TIDAK diubah demi tetap sederhana & konvensional (Principle V) — mengganti jadi lookup manual berbasis slug murni supaya bisa 100% ter-cache adalah kompleksitas tambahan yang tidak sepadan untuk starter kit ini. Konsekuensinya: yang benar-benar dibungkus `rememberPublicPage()` di `show()` adalah **query turunan/sekunder** (`relatedProducts`, `$related` artikel, relasi `portfolioCategory`) — bukan fetch model utamanya. Ini tetap memenuhi FR-005 (data turunan disimpan sementara) dan tetap aman untuk FR-008 (model utama SELALU fresh per request lewat binding, bukti tambahan bahwa halaman detail tidak pernah menampilkan data utama basi). Cache yang dibungkus di `index()` (Home/Produk/Artikel/Portfolio/FAQ/Tentang Kami) TIDAK punya keterbatasan ini — seluruhnya query eksplisit yang 100% ter-cache, manfaat penuh.
